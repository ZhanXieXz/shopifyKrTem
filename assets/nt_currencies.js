  ...
  geckoShopify.Currency_convertAll = function(oldCurrency, newCurrency, selector, format) {
      ...
      $(selector || 'span.money' ).each(function() {
        ...
        if (!$this.attr(dt_cur_shop)) {
            ...
            // }
            if (hover_currency){ 
              $this.wrap('<span class="ttip_nt tooltip_top"></span>');
              $this.parent().append('<span class="tt_txt">' + getHtmlPrice + '</span>'); 
            }
        }
...
//     if ($('#menu_mb_cat').find('#item_mb_wis').length <= 0) {
//       let mb_menu = $('#nt_menu_canvas');
//       if (mb_menu) {
//         console.log('将mobie menu的项合并到mobile Categories');
//         $('.mb_nav_title[data-id="#shopify-section-mb_nav_js"]').hide();
//         $('.mb_nav_title[data-id="#shopify-section-mb_cat_js"]').css({
//           flex: '0 0 100%',
//           maxWidth: '100%'
//         });
//         const loginBtn = mb_menu.find('#item_mb_acc');
//         $('#menu_mb_cat').prepend(loginBtn);
//         $('#menu_mb_cat').append($('#menu_mb_ul').html())
//       }
//     }
//   })
